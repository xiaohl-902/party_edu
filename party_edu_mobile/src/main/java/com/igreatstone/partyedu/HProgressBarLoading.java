...
public class HProgressBarLoading extends View {
    ...
    private void initAttr(AttributeSet attrs) {
        TypedArray array = mContext.obtainStyledAttributes(attrs,
                R.styleable.hprogress);
        mMax = array.getInt(R.styleable.hprogress_max, 10);
        mCurProgress = array.getInt(R.styleable.hprogress_progress, 0);
        mDefaultHeight = array
                .getInt(R.styleable.hprogress_progressHeight, 200);
        mColor = array.getColor(R.styleable.hprogress_progressColor,
                Color.parseColor("#0AC416"));
        array.recycle();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.EXACTLY) {
            ...
            // mDefaultWidth 为你自定义设置的属�?
            mWidth = UiUtil.dp2px(mContext, 300);
        ...
        if (heightMode == MeasureSpec.EXACTLY) {
            ...
            // mDefaultHeight 为你自定义设置的属�?
            mHeight = UiUtil.dp2px(mContext, mDefaultHeight);
        }
    ...
    public void setCurProgress(int curProgress, long time,
                               final OnEndListener listener) {
        if (mCurProgress == 100) {// 重置mCurProgress�?
            mCurProgress = 0;
        }
        ...
        animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                mCurProgress = (Integer) animation.getAnimatedValue();
                postInvalidate();// 通知刷新
            }
        });
        animator.start();
